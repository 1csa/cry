...
import { getTaskMicro} from '../../services/recommended'
const { TextArea } = Input;
interface Props {
    location : {state : any}
}

function u2mircoPloem(props: Props) {
    const { } = props
  ...
  useEffect(() => {
    ...
    const handTaskList = async(data:any) => {
        setLoading(true)
        let res = await getTaskMicro({params:{url:data.URL.trim() , _t:blenderListNum.startTime}})
        if(res?.code == 0) {
            //列表
            ...
            setBlenderListNum((item: any) => ({
                ...
                blender_num:res.result[0].blender_num
            }))
            setLoading(false)
    ...
    const getRecalls = [
        ...
        },
        {
            ...
            render: (value: any, record: any) => {
              ...
              } else if (record.name.indexOf("aiorecall") != -1 || record.name.indexOf("allinone") != -1) {
                return (
                  <div style={{ display: "flex" }} key={record.id}>
                    <Tooltip placement="bottom" title={value} >
    ...
    return (
        <div className="BlenderPloem-content" >
            ...
            <h4 style={{ paddingLeft: 10 }}>user2micro问题排查</h4>
                ...
                <div style={{padding:10}}>
                    <Tag style={{fontSize:14 , marginBottom:10}} color="cyan">第三步：blender > {blenderListNum.blender_num}</Tag>
