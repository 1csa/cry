...
import './index.less'
import { getTaskVideo} from '../../services/recommended'
const { TextArea } = Input;
interface Props {
    location : {state : any}
}

function videoPloem(props: Props) {
    const { } = props
    ...
    const [afterpreprocessor , setAfterpreprocessor] = useState<any>([])
    const [predictitem , setPredictitem] = useState<any>([])
    ...
    const handTaskList = async(data:any) => {
        setLoading(true)
        let res = await getTaskVideo({params:{url:data.URL.trim() , _t:blenderListNum.startTime}})
        if(res?.code == 0) {
            //列表
            ...
            setAfterpreprocessor(res.result[0].afterpreprocessor)
            setPredictitem(res.result[0].predictitem)
            //数值
            setBlenderListNum((item: any) => ({
                ...
                merge : res.result[0].merge,
                blenderNum : res.result[0].blender_num,
                afterpreprocessor_num : res.result[0].afterpreprocessor_num,
                predictitem_num : res.result[0].predictitem_num
            }))
            setLoading(false)
    ...
    return (
        <div className="BlenderPloem-content" >
            ...
            <h4 style={{ paddingLeft: 10 }}>user2video问题排查</h4>
                ...
                {/*    <Table style={{width:"100%"}}  scroll={{ x: 800 }} dataSource={filters} columns={getFilters} pagination={false} loading={loading}/>*/}
                {/*</div>*/}
                <div style={{padding:10}}>
                ...
                {/*<div style={{padding:10}}>*/}
                {/*    <Tag style={{fontSize:14 , marginBottom:10}} color="cyan">第四步：merge > {blenderListNum.merge}</Tag>*/}
                {/*</div>*/}
                <div style={{padding:10}}>
                    <Tag style={{fontSize:14 , marginBottom:10}} color="cyan">第五步：过滤 > {blenderListNum.packager_docs_num}</Tag>
                ...
                {/*</div>*/}
                {/*<div style={{padding:10}}>*/}
                {/*    <Tag style={{fontSize:14 , marginBottom:10}} color="cyan">过滤后剩余文章 > {blenderListNum.afterpreprocessor_num}</Tag>*/}
                {/*    <Table  dataSource={afterpreprocessor}  pagination={false}  columns={getAfterpreprocessor} loading={loading}/>*/}
                {/*</div>*/}
                {/*<div style={{padding:10}}>*/}
                {/*    <Tag style={{fontSize:14 , marginBottom:10}} color="cyan">第六步：打分 > {blenderListNum.predictitem_num}</Tag>*/}
                {/*    <Table  dataSource={predictitem} columns={getPredictitem} pagination={false} loading={loading}/>*/}
                {/*</div>*/}
                <div style={{padding:10}}>
