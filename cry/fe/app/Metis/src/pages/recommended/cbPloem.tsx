...
import './index.less'
import { getTaskCb} from '../../services/recommended'
const { TextArea } = Input;
interface Props {
    location : {state : any}
}

function cbPloem(props: Props) {
    const { } = props
    ...
    const handTaskList = async(data:any) => {
        setLoading(true)
        let res = await getTaskCb({params:{url:data.URL.trim() , _t:blenderListNum.startTime}})
        if(res?.code == 0) {
            //列表
    ...
    return (
        <div className="BlenderPloem-content" >
            ...
            <h4 style={{ paddingLeft: 10 }}>cb问题排查</h4>
