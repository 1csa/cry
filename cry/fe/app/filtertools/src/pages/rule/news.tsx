...
import { Card, Table, message, Button, Modal, Input, Form ,Pagination} from 'antd';
import request from '@/utils/request';
import axios from 'axios';
import AddCoverForm from '@/components/newsRule/addForm';
import { LogInfo, defaultNewRuleFormValue, newscolumns } from '@/config/constant';
...
import {request_section_msg} from "editor-push-news/src/config/editorpush/push.config";

const isDev = getIsDev();
const defaultFormValue = defaultNewRuleFormValue;
const { Search } = Input;
interface GenerationProps {
  form: { getFieldDecorator: any; validateFields: any; resetFields: any ;setFieldsValue:any};
  location: {query: any, params: any};
}
const NewsRule: FC<GenerationProps> = (props:GenerationProps) => {
  const column = [
    {
      title: '编号',
      dataIndex: 'ruleId',
      ...
      render: (record: any) => (
        <span>
          ...
          <Button
            ...
            size="small"
            icon="delete"
            onClick={() => deleteNewsRulePop(record)}
          >
            删除
          </Button>
        </span>
  ...
  useEffect(() => {
    ...
    props.location.query.ruleName?searchRule():getAllNewsRules();
  }, []);

  const getCategoryList = async () => {
    const ret = await axios.get(
      `/api/proxy/http://operationtoolservice.int.yidian-inc.com/OperationtoolService/GetCategories?appid=yidian`,
    );
    if (ret.data.code === 0) {
      setCategoryList(ret.data.result);
    } else {
      message.error(`大类获取失败,请刷新重试`);
      setCategoryList([]);
    }
  };
  const getAllNewsRules = async (currents, pageSizes) => {
    const ret = await request.get(`/api/filtertools/getAllNewsRules?isdev=${isDev}&current=${currents ? currents : current}&pageSize=${pageSizes ? pageSizes : pageSize}`);
    if (ret.code === 0) {
      const filterResult = ret.result.data.filter( (item:any) => item.newsDelStatus !== true)
  ...
  //获取图文导出数据
  const getAllNewsRulesExport = async () => {
    const ret = await request.get(`/api/filtertools/getAllNewsRulesExport?isdev=${isDev}`);
    if (ret.code === 0) {
      const filterResult = ret.result.filter( (item:any) => item.newsDelStatus !== true)
      setResultExport(filterResult)
    } else {
      message.error(`统计数据获取失败,${'原因: ' + ret.message ? ret.message : ''},请刷新重试`);
    }
  };
  ...
  const getAllNewsThreshold = async () => {
    const ret = await request.get(`/api/filtertools/getAllNewsThreshold?isdev=${isDev}`);
    if (ret.code === 0) {
      let handleThreshold = {};
  ...
  const editNewsRule = (data: any) => {
    ...
    if (
      data.channelList &&
      data.channelList.content &&
      typeof data.channelList.content !== 'string'
    ) {
  ...
  const deleteNewsRule = async (data: any) => {
    ...
    const ret = await request.post(`/api/filtertools/deleteNewsRule?isdev=${isDev}`, {
      data: {
        ruleId: data.ruleId,
        userName
      },
    ...
    if (ret.code === 0) {
      const { log_domain, log_secret, news_rule_key }: any = LogInfo;
      ...
      saveKibana(act, news_rule_key, value);
      let newresult: any[] = [];
      newresult = newresult.concat(result);
      newresult.splice(
        newresult.findIndex(item => item.ruleId === data.ruleId),
        1,
      );
      setResult(newresult);
  ...
  const searchRule = async () => {
    ...
    validateFields((err:any, value:any) => {
       ...
       request.post(`/api/filtertools/${port}?isdev=${isDev}`, {
        ...
        if (res.code === 0) {
          const newData = res.data.data.filter((item:any) => !item.newsDelStatus);
          if(newData.length > 0) {
            setResult(newData)
  ...
  const onExportData = () => {
    exportData('图文过滤规则表', resultExport, newscolumns);
  };
      ...
      <div className="main-content">
        <Card bordered={false} style={{ minHeight: 380}}>
          <div style={{ marginBottom: '10px' ,display:'flex',alignItems:'center'}}>
            ...
            <div style={{paddingLeft : 100}}>图文过滤规则: 共{result.length}条数据</div>
          </div>
          <Table columns={column} loading={loading} dataSource={result} pagination={false} rowKey="_id"/>
          <Pagination
...
const GenerationDemo = Form.create({ name: 'Check_other' })(NewsRule);
