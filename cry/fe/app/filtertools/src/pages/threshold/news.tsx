...
import { FormItemLayout, TailFormItemLayout, ThresholdSelectCon, NewsThresholdlistName, LogInfo } from '@/config/constant';
...
const NewsThreshold: FC<FormProps> = (props: FormProps) => {
  ...
  const [currenttab, setCurrenttab] = useState<string>(NewsThresholdlistName[0].key);
      ...
      if (!err) {
        Modal.confirm({
          ...
          onOk(){
            ...
            NewsThresholdlistName.map(item =>{
  ...
  const submitAction = async(data) => {
    // console.log(data)
    const ret = await request.post(`/api/filtertools/updateNewsThreshold?isdev=${isDev}`, {
      data: data
    });
    const act = 'save',
          value = data;
    ...
    if (ret.code === 0) {
      ...
      const { log_domain, log_secret, news_threshold_key} = LogInfo
      saveStaticLog(log_domain, log_secret, [{
        action: act,
        key: news_threshold_key,
        value: value ? JSON.stringify(value) : '',
        fromid: '',
        comment: '',
      }])
      saveKibana(act,news_threshold_key,value)
    } else {
  ...
  const changeTab = (key) => {
    let tabkey = NewsThresholdlistName[key-1].key;
    ...
    <>
      <div className="main-content">
        <Card bordered={false} style={{ minHeight: 380 }}>
          <h4 style={{marginBottom: '20px'}}>图文过滤阈值设置</h4>
        ...
        <AddThresholdForm
          ...
          threshold={threshold}
          listName={NewsThresholdlistName}
