        ...
        return (
            <div className="audit-doc-wrapper">
                <div className="doc-title">
                    <span>{index+1}.&nbsp;&nbsp;</span>
                    <Button type="primary" href={`https://www.baidu.com/s?wd=${doc.title}`} target="_blank">百度一下</Button>
                    <a href={`https://www.yidianzixun.com/article/${doc.docid}`} target="_blank">{doc.title}</a>
                </div>
                <div className="doc-info">
                    {doc.wemedia && <Tag color="#2db7f5">自媒体</Tag>}
                    {doc.stky && <Tag color="red">置顶</Tag>}
                    {doc.sel && <Tag color="orange">加精</Tag>}
                    <span><strong>大类: </strong>{doc.cat_class && doc.cat_class.join(',')}</span>
                    <span><strong>文章源: </strong>{doc.source}</span>
                ...
                <div className="doc-comments">
                    {
                        comments.map(
                            (item:any, index:number) =>  <CommentDoc 
                                key={item.id} 
                                index={index} 
                                docid={doc.docid}
                                comment={item} 
                ...
                <div className="doc-oper">
                    ...
                    <div>
                        ...
                        <div className="oper-btns">
                            <Button style={{backgroundColor: '#2fda2f', color: '#fff'}} onClick={this.onPassArticle}>通过</Button>   
                            <Button style={{backgroundColor: '#ee82ee', color: '#fff'}} onClick={this.onHideArticle} disabled={!label}>不展示</Button>  
                            <Button style={{backgroundColor: '#ff8c00', color: '#fff'}} onClick={this.onNoserveArticle} disabled={!label}>不服务</Button>  
                            <Button style={{backgroundColor: '#d2691e', color: '#fff'}} onClick={this.onNorecArticle} disabled={!label}>不推荐</Button> 
                            <Button onClick={this.onDeleteArticle} disabled={!label}>删除</Button> 
                        </div>
                    </div>
                </div>
            </div>
