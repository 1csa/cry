...
import { Card, Form, DatePicker, Input, Select, Table, message, Button, Icon, Tag, Popconfirm, Pagination } from 'antd';
...
const BlackListPage: FC<FormProps> = (props: FormProps) => {
  ...
  const confirmDelete = async (record) => {
    const ret = await axios.post(`/api/proxy/${appConfig.API_HOST}/nameList/updateStatus`, {id: record.id, createUser: decodeURIComponent(getCookie('nickname'))});
    if(ret.data.status === 200) {
      let newList: any[] = [];
  ...
  const children = <Button type="primary" icon="plus" onClick={addNewFeature}>加黑名单</Button>
  return (
    <> 
      <div className="main-content">
        <PageHeader current="黑名单管理" home="黑白名单" children={children}/>
