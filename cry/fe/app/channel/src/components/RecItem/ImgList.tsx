...
interface IImgItemProps {
  ...
  index: number;
  doc: RecItemProps;
  updateTopItem: (index: number, topItem: RecItemProps) => void;
}
    ...
    setTimeout(async () => {
      ...
      if (res.status === 'success') {
        ...
        if (uploadIndex === -1) {
          ...
          imgArr.splice(uploadIndex, 1, res.data.url);
        }
        setFileList(fileList.slice(0));
        setImgArr(imgArr);
        updateDocImage({ docid });
        if (imgArr.length === 0 || imgArr.length === 1 || imgArr.length === 3) {
          
          setImages({ docid, imageUrls: imgArr.join(), covertype: 'normal' }).then(res => {
            message.success('封面图更新成功！');
          });
        } else {
          message.info('文章封面图只能为无图、单图或三图');
        }
        let tempDoc = { ...doc };
...
const mapDispatchesToProps = (dispatch: Dispatch) => ({
  updateTopItem: topNewsDispatches.updateRecItem(dispatch),
});
