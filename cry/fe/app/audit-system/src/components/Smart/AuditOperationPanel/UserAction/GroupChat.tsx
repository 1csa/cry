...
const GroupAction: React.FC<Omit<UserActionProps, 'type'>> = ({ data, reloadCallBack, checkedMessageIds = [] }) => {
  ...
  const submitAudit = (status: boolean) => {
    ...
    const auditHitMessages: any[] = [];
    material.messages.forEach((item: any, index: number) => {
      const { messagesId = index } = item;
      const checked = checkedMessageIds.includes(messagesId);
      checked && auditHitMessages.push(item);
    });

    const { manual_result: manualResult } = requestData;
    const manual_result = JSON.parse(manualResult);
    manual_result.auditHitMessages = auditHitMessages;
    requestData.manual_result = JSON.stringify(manual_result);
