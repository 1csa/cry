...
import { Card, Button, Tag, Modal, Drawer, Popover, Empty, Image } from 'antd';
...
import { formateTime, getCookie } from '@/utils/dev_helper';
import { reviewStatus, SubBusinessType } from '@/data/constants';
...
const CommentPanel: React.FC<ICommentPanelProps> = ({ commentInfo, tagOptions, reloadCallBack }) => {
  const isHis = location.pathname.includes('shortContent/historicalContentList');

  // 子业务
  const businessUnitType = tagOptions.business_unit_type[0] ?? '';
  // 啫喱 群名称
  const isGroup = businessUnitType === SubBusinessType.zenLyGroup;
  // 啫喱 想法/动态
  const isDynamic = businessUnitType === SubBusinessType.zenLyDynamic;
  const groupDynamic = isGroup || isDynamic;
  ...
  const formatItemData = (item: any, res: any) => {
    // 所有可能出现敏感词的部分
    const commentWords = item?.machine_result?.sensitive?.comment?.words ?? [];
    const { matchWordsToHtml, category } = getCommentWords(commentWords, res.sensitiveHighlightList);
    // console.log('function comment panel formatItemData ---->>', item, matchWordsToHtml, category);
    ...
    if (commentWords.length > 0) {
      ...
      // 如果有父级短文本 也高亮
      // if (item.material.parentComment) {
      //   item.material.parentComment.content = highlighComment(
      //     matchWordsToHtml,
      //     item?.material?.parentComment?.content,
      //   );
      // }
    }
    return item;
  ...
  const titleInfo = (article: any, cmtid: string, part_zone_cn: string, dataId: string) => (
    <div className="jcsb mb20">
      ...
      <div className="tags">
        <BasicCopyToClipboard
          data={cmtid}
          title={`${isGroup ? '群聊名称' : isDynamic ? '想法' : '短文本'}ID`}
          renderChild={() => <Tag className="cp">ID：{cmtid}</Tag>}
        />
  ...
  const commentContentPanel = (
    ...
    return (
      <div className={`${hasMarLeft ? 'ml74' : ''}`}>
        <p dangerouslySetInnerHTML={{ __html: mainContent }} className="warp-text"></p>
            ...
            {images?.map((ele, indx) => {
              return (
                <Popover
                  key={ele?.id}
                  placement="right"
                  content={<img style={{ width: 500, height: 500, objectFit: 'contain' }} src={ele?.url} alt="文章图片信息" />}
                  title="图片预览"
                >
                  <img className="mr10 mb20 content-img obf" src={ele?.url} />
                </Popover>
              );
            })}
          </div>
