...
import { moduleOptions, degreeOptions } from '@/data/constants';

import { UserActionProps } from './index';

const MessageAction: React.FC<Omit<UserActionProps, 'type'>> = ({ data, reloadCallBack }) => {
  const { material } = data;
  ...
  const [modules, setModules] = useState(moduleOptions);
  ...
  useEffect(() => {
    ...
    setSbmitBtnStatus(!moduleSelected && !degreeSelected);
  }, [modules, degrees]);
